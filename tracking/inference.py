...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        num_for_each = self.numParticles/len(legalPositions)
        for i in legalPositions:
            for j in range(num_for_each):
                samples.append(i)
        remaining = self.numParticles%len(legalPositions)
        random.shuffle(legalPositions)
        samples += legalPositions[0:remaining]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for pos in self.samples:
                weights[pos] += emissionModel[util.manhattanDistance(pos,pacmanPosition)]
            ...
            if weights.totalCount() == 0:
                ...
                for i in range(self.numParticles):
                    samples.append(util.sample(weights))
                self.samples = samples
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        samples = []
        for oldPos in self.samples:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            newPos = util.sample(newPosDist)
            samples.append(newPos)
        self.samples = samples
        # util.raiseNotDefined()
...
class JointParticleFilter:
        ...
        for s in self.samples:
        	...
        	for ghost in range(self.numGhosts):
        		if noisyDistances[ghost] is not None:
        			temp *= emissionModels[ghost][util.manhattanDistance(s[ghost],pacmanPosition)]
        	weights[s] += temp
        ...
        if weights.totalCount() == 0:
       	...
       	for ghost in range(self.numGhosts):
       		if noisyDistances[ghost] is None:
       			self.samples = [self.getParticleWithGhostInJail(i, ghost) for i in self.samples ]
    ...
    def elapseTime(self, gameState):
        ...
        """
        newParticles = []
        for oldParticle in self.samples:
